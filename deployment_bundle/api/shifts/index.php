...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    // List Shifts
    $sql = "SELECT * FROM shifts WHERE user_id = ?";
    $params = [$userId];
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO shifts (user_id, location_name, location_address, start_time, end_time, payment_amount, tax_percentage, shift_type, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
            $userId,
