...
const Workplaces: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!newItem.name.trim()) {
            setFieldErrors({ name: 'Nome do local é obrigatório' });
            return;
        }
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                        <div className="grid md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input
                                    type="text"
                                    className={`w-full px-4 py-2 rounded-lg border ${fieldErrors.name ? 'border-red-500 ring-1 ring-red-500' : 'border-slate-200'} focus:ring-2 focus:ring-blue-500 outline-none transition-colors`}
